...
## 1. Load Blancing và các khái niệm liên quan
...
- Một bộ cân bằng tải có thể hoạt động trên :
	- Link level : được gọi là link load blancing, nó gồm việc chọn các network link để gửi các các packet
	- Network level : được gọi là network load blancing, nó bao gồm các công việc chọn tuyền đường  mà các packet sẽ đi ( packet flow ) 
	- Server level : được gọi là server local blancing,  nó bao gồm các công việc xác định server nào sẽ xử lý tác vụ hoặc request . 
...
### 3.2 . Keep Alive trong HA

- Đối với những mô hình yêu cầu tinh sẵn sàng cao, Keep alive thường được sử dụng **HA IP Failover** 
- Tính sẵn sàng cao trong Keep Alive sử dụng giao thức VRRP ( Virtual Redudancy Routing Protocol ) 
- Trong HA, keep alive cung cấp một IP VIP  ( IP Failover ) cho một Cluser . Cho phép các bộ cân bằng tải hoạt động theo cơ chế Active-Backup. Nếu bộ load blancer chính bị down , thì IP Floating sẽ được chuyển về về bộ load blancer phụ 
